...
export default function FederationPolicyManager() {
  ...
  const [blocklist, { setData, refetch }] = useAsyncResource<BlockedInstancesResponse | null>(fetcher);

  const sortedEntries = useMemo(() => {
    const entries = blocklist.data?.blocked_instances ?? [];
  ...
  const handleRemove = async (domain: string) => {
    ...
    try {
      ...
      setData(res);
      setMessage(
        res.still_blocked ? `${domain} は環境変数によって引き続きブロックされています。` : "ブロックを解除しました。",
      );
    } catch (err) {
      if (err instanceof ApiError && err.status === 403) {
        setForbidden(true);
  ...
  const sourceLabel = useMemo(() => {
    const source = blocklist.data?.source;
    if (!source) return "";
  ...
  return (
    <section className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
      <div className="flex items-start justify-between gap-3 mb-4">
        <div>
          <h2 className="text-lg font-semibold">Federation Policy</h2>
      ...
      {!forbidden && (
        <>
          <div className="mb-4">
            <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">ブロックリスト</h3>
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">ブロックしたインスタンスからのアクティビティは拒否されます。</p>
          </div>
          ...
          <div className="border border-gray-200 dark:border-gray-700 rounded-lg">
            {blocklist.loading ? (
              ...
              <div className="p-6 text-center text-sm text-gray-600 dark:text-gray-400">現在ブロックしているインスタンスはありません。</div>
            ) : (
              <ul className="divide-y divide-gray-200 dark:divide-gray-700">
                {sortedEntries.map((entry) => (
                  <li key={entry.domain} className="flex items-center justify-between gap-3 px-4 py-3">
                    <div>
                      <div className="font-mono text-sm break-all">{entry.domain}</div>
                      <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 mt-1">
                        <span className="px-2 py-1 rounded-full border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
                          {entry.source === "env"
                            ? "ENV (読み取り専用)"
                    ...
                    <div className="flex items-center gap-2">
                      {entry.env && !entry.config && (
                        <span className="text-xs text-gray-500 dark:text-gray-400">環境変数で固定</span>
                      )}
                      {entry.config && (
                        <button
                          type="button"
