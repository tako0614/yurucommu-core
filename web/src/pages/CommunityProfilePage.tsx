...
const communityProfile_activeTabAtom = atom<'about' | 'members' | 'settings'>('about');
...
export function CommunityProfilePage() {
  ...
  const [joinRequests, setJoinRequests] = useAtom(communityProfile_joinRequestsAtom);
  const [loadingRequests, setLoadingRequests] = useAtom(communityProfile_loadingRequestsAtom);
  const [inviteCode, setInviteCode] = useAtom(communityProfile_inviteCodeAtom);
  const [creatingInvite, setCreatingInvite] = useAtom(communityProfile_creatingInviteAtom);
  const [requestAction, setRequestAction] = useAtom(communityProfile_requestActionAtom);
  const [memberActionError, setMemberActionError] = useAtom(communityProfile_memberActionErrorAtom);
  const [updatingMemberRole, setUpdatingMemberRole] = useAtom(communityProfile_updatingMemberRoleAtom);
  // Settings state
  const [settingsForm, setSettingsForm] = useAtom(communityProfile_settingsFormAtom);
  const [savingSettings, setSavingSettings] = useAtom(communityProfile_savingSettingsAtom);
  const [settingsError, setSettingsError] = useAtom(communityProfile_settingsErrorAtom);
  const [uploadingIcon, setUploadingIcon] = useAtom(communityProfile_uploadingIconAtom);
