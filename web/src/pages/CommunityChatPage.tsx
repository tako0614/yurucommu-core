    ...
    <div class="fixed inset-0 bg-neutral-900/80 z-50 flex items-center justify-center p-4">
      <div class="bg-neutral-900 rounded-xl w-full max-w-md max-h-[80vh] flex flex-col">
        <div class="flex items-center justify-between p-4 border-b border-neutral-800">
          <h2 class="text-lg font-bold text-white">メンバー ({props.members.length})</h2>
          <button onClick={props.onClose} aria-label="Close" class="text-neutral-400 hover:text-white">
            <CloseIcon />
          </button>
        </div>
...
export function CommunityChatPage() {
  ...
  const [errorMessage, setErrorMessage] = useAtom(communityChat_errorMessageAtom);
  ...
  return (
    <Show when={!loading()} fallback={
      ...
      <Show when={community()} fallback={
        <div class="flex flex-col items-center justify-center h-full bg-neutral-900">
          <div class="text-neutral-500 mb-4">{t('communityChat.notFound')}</div>
          <button
        ...
        <Show when={community()!.is_member} fallback={
          <div class="flex flex-col items-center justify-center h-full bg-neutral-900">
            <div class="text-neutral-500 mb-4">{t('communityChat.notMember')}</div>
            <button
              onClick={() => navigate('/groups')}
              class="px-4 py-2 bg-neutral-800 text-white rounded-lg hover:bg-neutral-700 transition-colors"
            >
            ...
            <header class="sticky top-0 bg-neutral-900/90 backdrop-blur-sm z-10 border-b border-neutral-900">
              <div class="flex items-center gap-3 px-2 py-2">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="text-xs text-neutral-500">{community()!.member_count}人</div>
                </div>
            ...
            <div class="flex-1 overflow-y-auto px-4 py-4">
              ...
              <Show when={messages().length > 0} fallback={
                <div class="text-center text-neutral-500 py-8">
                  ...
                  <p class="text-sm mt-2">{t('communityChat.noMessagesHint')}</p>
                </div>
              }>
                    ...
                    <div>
                      ...
                      <For each={group.messages}>
                        {(msg, msgIndex) => {
                          ...
                          return (
                            <div
                              class={`flex gap-2 mb-1 ${isMe ? 'flex-row-reverse' : ''}`}
                            >
                              <Show when={!isMe}>
                                <div class="w-8 flex-shrink-0">
                                  <Show when={showAvatar}>
                                    <A href={`/profile/${encodeURIComponent(msg.sender.ap_id)}`}>
                                      <UserAvatar
                              ...
                              <div class={`flex flex-col ${isMe ? 'items-end' : 'items-start'} max-w-[70%]`}>
                                ...
                                <div class="flex items-end gap-2">
                                  <Show when={isMe}>
                                    <span class="text-xs text-neutral-600">{formatTime(msg.created_at)}</span>
                                  </Show>
                                  <div
