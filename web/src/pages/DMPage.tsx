  ...
  return (
    <div class="flex items-start gap-3 p-4 border-b border-neutral-800">
      <img
        ...
        alt={props.request.sender.name || props.request.sender.preferred_username}
      ...
      <div class="flex-1 min-w-0">
        ...
        <p class="text-sm text-neutral-400 mt-1 line-clamp-2">{props.request.content}</p>
  ...
  const handleRead = () => {
    ...
    if (sc?.type === 'user') {
      setContacts(prev => prev.map(c =>
        c.ap_id === sc.ap_id ? { ...c, unread_count: 0 } : c
      ));
    }
  };
        ...
        <>
          ...
          <header class="sticky top-0 bg-neutral-900/95 backdrop-blur-sm z-10">
            ...
            <div class="flex items-center justify-between px-4 py-3">
              ...
              <div class="flex items-center gap-2">
                {/* Search icon */}
                <button aria-label="Search" class="p-2 text-neutral-400 hover:text-white transition-colors">
                  <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                  </svg>
                </button>
                {/* New chat icon */}
                <button aria-label="New chat" class="p-2 text-neutral-400 hover:text-white transition-colors">
                  <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                  </svg>
                </button>
                {/* More icon */}
                <button aria-label="More options" class="p-2 text-neutral-400 hover:text-white transition-colors">
          ...
          >
            ...
            <Show when={!loading() && activeTab() === 'requests'}>
              <Show when={requests().length === 0} fallback={
                ...
                <div class="flex-1 flex flex-col items-center justify-center p-8 text-center min-h-[50vh]">
                  ...
                  <p class="text-neutral-400 mb-2 text-lg font-medium">リクエストがありません</p>
            ...
            <Show when={!loading() && activeTab() !== 'requests' && currentContacts().length === 0}>
              <div class="flex-1 flex flex-col items-center justify-center p-8 text-center min-h-[50vh]">
                <div class="w-20 h-20 mb-4 rounded-full bg-neutral-800 flex items-center justify-center">
                  <svg class="w-10 h-10 text-neutral-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
