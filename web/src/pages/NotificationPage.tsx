...
export function NotificationPage() {
  ...
  createEffect(() => {
    ...
    const loadNotifications = async () => {
      try {
        const data = await fetchNotifications({ type: currentFilter === 'all' ? undefined : currentFilter });
        if (cancelled) return;
  ...
  const getNotificationText = (notification: Notification): JSX.Element => {
    ...
    switch (notification.type) {
      case 'follow':
        return <><span class="font-bold text-white">{actorName}</span>{t('notifications.follow')}</>;
      case 'follow_request':
        return <><span class="font-bold text-white">{actorName}</span> sent you a follow request</>;
      case 'like':
        return <><span class="font-bold text-white">{actorName}</span>{t('notifications.like')}</>;
      case 'announce':
        return <><span class="font-bold text-white">{actorName}</span>{t('notifications.repost')}</>;
      case 'mention':
        return <><span class="font-bold text-white">{actorName}</span>{t('notifications.mention')}</>;
      case 'reply':
        return <><span class="font-bold text-white">{actorName}</span>{t('notifications.reply')}</>;
      default:
        return <>New notification</>;
    }
  };

  const getNotificationIcon = (type: Notification['type']): JSX.Element | null => {
    switch (type) {
      case 'follow':
        return <div class="p-1 bg-blue-500 rounded-full text-white"><FollowIcon /></div>;
      case 'follow_request':
        return <div class="p-1 bg-yellow-500 rounded-full text-white"><FollowRequestIcon /></div>;
      case 'like':
        return (
      ...
      case 'mention':
        return <div class="p-1 bg-purple-500 rounded-full text-white"><MentionIcon /></div>;
      case 'reply':
        return (
  ...
  const filterTabs: { key: FilterType; label: string; icon: JSX.Element }[] = [
    { key: 'all', label: 'すべて', icon: <span class="w-4 h-4 flex items-center justify-center text-xs">全</span> },
    { key: 'follow', label: 'フォロー', icon: <FollowIcon /> },
    { key: 'like', label: 'いいね', icon: <HeartIcon class="w-4 h-4" filled stroke={false} /> },
    { key: 'announce', label: 'リポスト', icon: <RepostIcon class="w-4 h-4" /> },
              ...
              {(notification) => (
                ...
                >
                  ...
                  <div class="flex-1 min-w-0">
                    <p class="text-[15px] text-neutral-400">{getNotificationText(notification)}</p>
