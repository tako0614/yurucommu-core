...
const BACKOFF_SERIES_SECONDS = [60, 120, 240, 480, 960, 1920, 3840, 7680] as const;
...
export async function computeDeliveryJobId(activityId: string, endpoint: string): Promise<string> {
...
export function computeRetryDelaySeconds(nextAttempt: number): number {
  ...
  const idx = Math.max(0, Math.min(nextAttempt - 1, BACKOFF_SERIES_SECONDS.length - 1));
...
export function safeParseIsoTimeMs(value: string | null | undefined): number | null {
