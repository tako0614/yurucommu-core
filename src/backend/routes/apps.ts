...
appsApiRoutes.post('/:name/deploy', requireBearerAuth('apps:deploy'), async (c) => {
  ...
  if (!/^[a-zA-Z0-9_-]{1,64}$/.test(appName)) {
    return c.json({ error: 'invalid_request', error_description: 'Invalid app name' }, 400);
  }
  ...
  if (!Array.isArray(body.files)) {
    return c.json({ error: 'invalid_request', error_description: 'files must be an array' }, 400);
  }

  if (body.files.length > MAX_FILES) {
    return c.json({ error: 'invalid_request', error_description: `Too many files (max ${MAX_FILES})` }, 400);
  }

  const files = body.files as Array<{ path?: unknown; content?: unknown; contentType?: unknown }>;
  ...
  for (const file of files) {
    if (typeof file.path !== 'string' || typeof file.content !== 'string') {
      return c.json({ error: 'invalid_request', error_description: 'Each file must have path (string) and content (base64 string)' }, 400);
    }
    ...
    if (!normalizedPath || normalizedPath.includes('..') || normalizedPath.startsWith('/')) {
      return c.json({ error: 'invalid_request', error_description: `Invalid file path: ${file.path}` }, 400);
    }
    ...
    try {
      ...
      return c.json({ error: 'invalid_request', error_description: `Invalid base64 content for file: ${file.path}` }, 400);
    }
...
appsServeRoutes.get('/:clientId/:appName/*', async (c) => {
  ...
  if (object) {
    const contentType = object.httpMetadata?.contentType ?? inferContentType(filePath);
    const headers = new Headers();
    headers.set('Content-Type', contentType);
    headers.set('Cache-Control', filePath.includes('/assets/') ? 'public, max-age=31536000, immutable' : 'public, max-age=3600');
    if (object.httpEtag) {
