...
stories.post('/view', async (c) => {
  ...
  try {
    ...
    const existing = await db.select()
      .from(storyViews)
      .where(and(eq(storyViews.actorApId, actor.ap_id), eq(storyViews.storyApId, apId)))
      .get();

    if (!existing) {
      await db.insert(storyViews).values({ actorApId: actor.ap_id, storyApId: apId, viewedAt: now });
    }
  } catch {
...
stories.post('/vote', async (c) => {
  ...
  const existingVote = await db.select()
    .from(storyVotes)
    .where(and(eq(storyVotes.storyApId, apId), eq(storyVotes.actorApId, actor.ap_id)))
    .get();
...
stories.post('/:id/share', async (c) => {
  ...
  const existing = await db.select()
    .from(storyShares)
    .where(and(eq(storyShares.storyApId, apId), eq(storyShares.actorApId, actor.ap_id)))
    .get();
  if (existing) {
