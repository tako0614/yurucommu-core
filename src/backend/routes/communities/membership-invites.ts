...
export function registerMembershipInviteRoutes(communitiesRouter: Hono<{ Bindings: Env; Variables: Variables }>) {
  ...
  communitiesRouter.get('/:identifier/invites', async (c: Context<{ Bindings: Env; Variables: Variables }>) => {
  ...
  communitiesRouter.post('/:identifier/invites', async (c: Context<{ Bindings: Env; Variables: Variables }>) => {
    const actor = c.get('actor');
    if (!actor) return c.json({ error: 'Unauthorized' }, 401);
  ...
  communitiesRouter.delete('/:identifier/invites/:inviteId', async (c: Context<{ Bindings: Env; Variables: Variables }>) => {
    const actor = c.get('actor');
    if (!actor) return c.json({ error: 'Unauthorized' }, 401);
