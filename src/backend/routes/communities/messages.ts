...
messagesRouter.get('/:identifier/messages', async (c) => {
  ...
  const policyError = checkPostPolicy(community.postPolicy || 'members', membership ?? null);
  if (policyError) {
    return c.json({ error: policyError }, 403);
  }

  // Query objects addressed to this community (via object_recipients)
  const recipients = await db.select({ objectApId: objectRecipients.objectApId })
...
messagesRouter.patch('/:identifier/messages/:messageId', async (c) => {
  ...
  const message = await db.select({ apId: objects.apId, attributedTo: objects.attributedTo })
    .from(objects)
...
messagesRouter.delete('/:identifier/messages/:messageId', async (c) => {
  ...
  await db.run(sql`DELETE FROM object_recipients WHERE object_ap_id = ${messageId}`);
