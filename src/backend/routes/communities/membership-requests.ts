...
export function registerMembershipRequestRoutes(communitiesRouter: Hono<{ Bindings: Env; Variables: Variables }>) {
  ...
  communitiesRouter.get('/:identifier/requests', async (c: Context<{ Bindings: Env; Variables: Variables }>) => {
  ...
  communitiesRouter.post('/:identifier/requests/accept', async (c: Context<{ Bindings: Env; Variables: Variables }>) => {
    const actor = c.get('actor');
    if (!actor) return c.json({ error: 'Unauthorized' }, 401);
  ...
  communitiesRouter.post('/:identifier/requests/reject', async (c: Context<{ Bindings: Env; Variables: Variables }>) => {
    const actor = c.get('actor');
    if (!actor) return c.json({ error: 'Unauthorized' }, 401);
