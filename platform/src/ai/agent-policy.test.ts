...
describe("agent tool policy", () => {
  ...
  it("enforces allowlist rules per agent type", () => {
    ...
    expect(isToolAllowedForAgent("dev", "tool.runAIAction")).toBe(true);
  });
  ...
  it("blocks config changes for non-admin agents", () => {
    ...
    expect(() => assertToolAllowedForAgent("dev", "tool.updateTakosConfig")).toThrow(/not allowed/);
  });
  ...
  it("blocks code changes for user/admin agents", () => {
    ...
    expect(() => assertToolAllowedForAgent("system", "tool.applyCodePatch")).toThrow(
      /not allowed/,
    );
  });
});
