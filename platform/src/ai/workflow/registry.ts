...
function validateWorkflowStep(step: WorkflowStep): string[] {
  ...
  switch (step.type) {
    case "ai_action":
      ...
      } else if (!step.config.actionId?.trim()) {
    ...
    case "tool_call":
      ...
      } else if (!step.config.toolName?.trim()) {
    ...
    case "condition":
      if (step.config.type !== "condition") {
        ...
        if (!step.config.expression?.trim()) {
          errors.push("config.expression is required for condition step");
        }
        if (!Array.isArray(step.config.branches) || step.config.branches.length === 0) {
    ...
    case "loop":
      if (step.config.type !== "loop") {
        ...
        if (typeof step.config.maxIterations !== "number" || step.config.maxIterations <= 0) {
          errors.push("config.maxIterations must be a positive number for loop step");
        }
        if (!Array.isArray(step.config.body) || step.config.body.length === 0) {
    ...
    case "parallel":
      if (step.config.type !== "parallel") {
        ...
        if (!["all", "any", "none"].includes(step.config.waitFor)) {
    ...
    case "human_approval":
      ...
      } else if (!step.config.message?.trim()) {
    ...
    case "transform":
      ...
      } else if (!step.config.expression?.trim()) {
 ...
 * デフォルトのワークフローレジストリ
 */
export const workflowRegistry: WorkflowRegistry = createWorkflowRegistry();
