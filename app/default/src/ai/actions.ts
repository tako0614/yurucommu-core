/**
 * AI Actions for Default App
 *
 * These are App-layer AI actions that use Core's AI Provider connection layer.
 * Actions: chat, summary, tag-suggest, translation, dm-moderator
 */
import type {
  AiAction,
  AiActionContext,
...
// Define AiAuditLogger type locally to avoid cross-package dependency
type AiAuditLogger = (entry: {
  actionId: string;
  providerId: string;
  model: string | null;
  policy: unknown;
  ...
  userId: string | null;
  status: "attempt" | "success" | "error" | "blocked";
  error?: string;
}) => void | Promise<void>;
...
const tagSuggestHandler: AiActionHandler<TagSuggestInput, TagSuggestOutput> = async (ctx, input) => {
  const objectIds = (input as any)?.objectIds ?? (input as any)?.object_ids;
  const objectContext = await resolveObjectPayload(ctx, objectIds);
  const text = normalizeText(input?.text);
...
const translationHandler: AiActionHandler<TranslationInput, TranslationOutput> = async (ctx, input) => {
...
const dmModeratorHandler: AiActionHandler<DmModeratorInput, DmModeratorOutput> = async (ctx, input) => {
...
// Export action IDs for use in routes
export {
  CHAT_ACTION_ID,
  SUMMARY_ACTION_ID,
