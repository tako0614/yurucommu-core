...
- routes `routes/rtc/index.ts`（`index.ts` の `mountCoreRoutes` に `app.route("/", rtcRoutes)`）:
  ...
  - `GET /api/rtc/socket` — browser WS upgrade。`?actor=&ticket=`を必須とし、Call DO自身が検証して使い切る。cookie-at-upgrade経路は持たない。
...
4. 以前この文書にあった「Phase 2 UI/deploy配線済み」という記録はcurrent sourceと一致しないため撤回した。実装する場合はproduct側の正とする情報・差分・owner gateを証拠とする。
